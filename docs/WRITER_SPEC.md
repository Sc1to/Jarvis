...
>
> **Issue schema:** each entry in `qa_issues` carries `type`, `description`, `severity`, and now also `quote` (the exact offending sentence/clause, verbatim from the scene — empty only for a true omission, where the scene is missing a required element outright rather than containing a bad line) and `fix` (grounded, actionable guidance — never a restatement of the problem). This was added because a bare `description` forced the Writer to re-derive both the exact passage at fault and the correct fact on every rewrite, which produced *new* contradictions on retry as often as it fixed the flagged one. Every rewrite path — attempt-2+ auto-retry and the manual "send findings to the Writer agent" directive — now also passes the scene's *current draft* into the Writer's context (`assemble_writer_context(current_draft=...)` → `## Current draft (revise this)`) with an instruction to make targeted, minimal edits rather than regenerating the scene blind from the brief.
>
> **`fix` has four shapes**, matched to the kind of issue — `QA_SYSTEM` spells out all four with a worked example each, because a single generic definition kept needing one exception bolted on per new failure mode encountered in practice:
> - *Wrong fact* (entity/continuity/voice/contract): the correct fact from the ledger or prior scenes, to substitute directly.
> - *Redundant restatement* (recap — see below): what to cut. Restating the same recap accurately is still a recap, so this is never a corrected rewording.
> - *Premature knowledge*: the scene treats something the ledger fixes for later (a future discovery, event, or identification) as already true. The fix is what the passage should convey instead, using only what the character could plausibly know or feel now — not an instruction to simply delete the beat.
> - *Omission* (`quote` is empty): what needs to be shown, and where, to satisfy the missing requirement.
>
> **Reference vs. recap:** scenes need to connect to each other, which is not the same thing as restating what already happened. `WRITER_SYSTEM`'s economy rules distinguish the two: a plain, one-clause reference to an established fact that the current action needs ("Clara collected her case from the porter") is fine; re-narrating how or when that fact became true ("...from the porter who had found it among the trunks the previous evening") is a recap, and because it's reconstructed from memory rather than copied it tends to drift into contradicting the original detail too. QA's redundancy check (point 5) only flags the latter — a bare necessary reference is not an error.
